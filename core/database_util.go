...
var (
	...
	configPrefix   = []byte("ntc-config-") // config prefix for the db
...
func WriteCanonicalHash(db ntcdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db ntcdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db ntcdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db ntcdb.Putter, hash common.Hash) error {
...
func WriteHeader(db ntcdb.Putter, header *types.Header) error {
...
func WriteBody(db ntcdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db ntcdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db ntcdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db ntcdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db ntcdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db ntcdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db ntcdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db ntcdb.Database) ntcdb.Database {
	return ntcdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db ntcdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db ntcdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db ntcdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
