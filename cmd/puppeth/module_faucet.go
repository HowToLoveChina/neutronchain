...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/ntc                                                            && \
	(cd $GOPATH/src/github.com/ntc && git clone --depth=1 https://github.com/ntc/go-ntc) && \
  go build -v github.com/ntc/go-ntc/cmd/faucet                                              && \
