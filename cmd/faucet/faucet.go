...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the ntc protocol")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for ntc and the faucet")
)
...
type request struct {
	...
	Account  common.Address     `json:"account"`  // ntc address being funded
	Time     time.Time          `json:"time"`     // Timestamp when te request was accepted
	Tx       *types.Transaction `json:"tx"`       // Transaction funding the account
}

// faucet represents a crypto faucet backed by an ntc light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // ntc protocol stack
	client *ntcclient.Client   // Client connection to the ntc chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the ntc light client protocol
	...
	client := ntcclient.NewClient(api)
...
// close terminates the ntc connection and tears down the faucet.
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Iterate over all the files and look for ntc addresses
		...
		if address == (common.Address{}) {
			websocket.JSON.Send(conn, map[string]string{"error": "No ntc address found to fund"})
			continue
		}
