...
#
# The cluster contains a bootnode, a gntc node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the gntc and
# swarm nodes
start_bootnode() {
...
# start_gntc_node starts a gntc node with --datadir pointing at <base-dir>/gntc
# and a single, unlocked account with password "gntc"
start_gntc_node() {
  local dir="${base_dir}/gntc"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get gntc account address"
  fi
  ...
  start_node "gntc" "${GETH_IP}" "$(which gntc)" ${args[@]}
}
...
create_account() {
  ...
  gntc --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
