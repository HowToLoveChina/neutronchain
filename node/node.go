...
func (n *Node) OpenDatabase(name string, cache, handles int) (ntcdb.Database, error) {
	if n.config.DataDir == "" {
		return ntcdb.NewMemDatabase()
	}
	return ntcdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
