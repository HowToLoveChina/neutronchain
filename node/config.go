...
import (
	...
	"github.com/ntc/go-ntc/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gntc is "gntc". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gntc", keep that.
	if name == "gntc" || name == "gntc-testnet" {
		name = "gntc"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gntc" instances.
var isOldGntcResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by gntc 1.4 are used if they exist.
	if c.name() == "gntc" && isOldGntcResource[path] {
		oldpath := ""
		if c.Name == "gntc" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-ntc-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
