...
func NewLesServer(eth *eth.ntc, config *eth.Config) (*LesServer, error) {
...
func newCostStats(db ntcdb.Database) *requestCostStats {
...
func getChtRoot(db ntcdb.Database, num uint64) common.Hash {
...
func storeChtRoot(db ntcdb.Database, num uint64, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], num)
	db.Put(append(chtPrefix, encNumber[:]...), root[:])
}

func makeCht(db ntcdb.Database) bool {
