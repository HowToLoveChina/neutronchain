...
import (
	...
	"github.com/ntc/go-ntc/common/math"
...
// PublicntcAPI provides an API to access ntc related information.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicntcAPI struct {
	b Backend
}

// NewPublicntcAPI creates a new ntc protocol API.
func NewPublicntcAPI(b Backend) *PublicntcAPI {
	return &PublicntcAPI{b}
}

// GasPrice returns a suggestion for a gas price.
func (s *PublicntcAPI) GasPrice(ctx context.Context) (*big.Int, error) {
	return s.b.SuggestPrice(ctx)
}

// ProtocolVersion returns the current ntc protocol version this node supports
func (s *PublicntcAPI) ProtocolVersion() hexutil.Uint {
...
func (s *PublicntcAPI) Syncing() (interface{}, error) {
...
//   keccak256("\x19ntc Signed Message:\n"${message length}${message}).
//
// This gives context to the signed message and prevents signing of transactions.
func signHash(data []byte) []byte {
	msg := fmt.Sprintf("\x19ntc Signed Message:\n%d%s", len(data), data)
	return crypto.Keccak256([]byte(msg))
}

// Sign calculates an ntc ECDSA signature for:
// keccack256("\x19ntc Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/ntc/go-ntc/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19ntc Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be be 27 or 28 for legacy reasons.
//
// https://github.com/ntc/go-ntc/wiki/Management-APIs#personal_ecRecover
func (s *PrivateAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != 65 {
		return common.Address{}, fmt.Errorf("signature must be 65 bytes long")
	}
	if sig[64] != 27 && sig[64] != 28 {
		return common.Address{}, fmt.Errorf("invalid ntc signature (V is not 27 or 28)")
	}
...
// PublicBlockChainAPI provides an API to access the ntc blockchain.
// It offers only methods that operate on public data that is freely available to anyone.
type PublicBlockChainAPI struct {
	b Backend
}

// NewPublicBlockChainAPI creates a new ntc blockchain API.
...
//
// https://github.com/ntc/wiki/wiki/JSON-RPC#eth_sign
...
// PublicDebugAPI is the collection of ntc APIs exposed over the public
// debugging endpoint.
...
// PrivateDebugAPI is the collection of ntc APIs exposed over the private
// debugging endpoint.
...
// Version returns the current ntc protocol version.
